...
#[inline(always)]
fn parse_16(s: &[u8]) -> Result<(u128, usize), AtoiSimdError> {
    let val = read_16(s);
...
pub(crate) fn parse_fb_64_pos<const MAX: u64, const LEN_MORE: usize>(
    ...
    let val = match len {
        ...
        8 => {
            let mut val_h = read_8(s.get_safe_unchecked(8..));
            len = (check_8(val_h) + 8).min(s.len());
            val_h = process_16(((val_h as u128) << 64) + val as u128, len) as u64;
            if len < 16 {
                return Ok((val_h, len));
            }
            val_h
...
pub(crate) fn parse_fb_128_pos<const MAX: u128>(s: &[u8]) -> Result<(u128, usize), AtoiSimdError> {
    ...
    let mut val = match len {
        ...
        8 => {
            ...
            len = (check_8(val_h) + 8).min(s.len());
            let val = process_16(((val_h as u128) << 64) + val as u128, len);
            if len < 16 {
                return Ok((val, len));
            }
            val
